...
async def delete_file(_, message):
    ...
    if len(args) > 1:
        link = args[1].strip()
    elif getattr(message, "reply_to_message", None) and getattr(
    ...
    if is_gdrive_link(link):
        LOGGER.info(
            f"Attempting GDrive delete: {link} (user: {getattr(user, 'id', None)})"
        )
        try:
            msg = await sync_to_async(
                GoogleDriveDelete().deletefile, link, getattr(user, "id", None)
            )
        except Exception as e:
            LOGGER.error(f"Error deleting GDrive file: {e}")
            msg = f"Error deleting Google Drive file: {e}"
    else:
        msg = "Please provide a valid Google Drive link either as an argument or by replying to a message containing the link."
