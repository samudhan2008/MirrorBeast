...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    bot_header = Config.CUSTOM_BOT_HEADER or "Fast Mirrors"
    bot_header_link = Config.CUSTOM_BOT_HEADER_LINK or "https://t.me/FastMirrors"
    ...
    for index, task in enumerate(
        ...
        msg += f"\n<blockquote expandable>╭ <b>Task By {task.listener.message.from_user.mention(style='html')} </b>"
    ...
    buttons = ButtonMaker()
    if not is_user:
        buttons.data_button("sᴛᴀᴛs", f"status {sid} ov", position="header")
